...
from ai_job_market.core import validate_input_schema, run_pipeline
...
def render_data_source_control(st, project_root: Path, role: str) -> Path:
    ...
    if role!="admin":
        st.caption("Standard-user mode uses the active validated workspace selected by the administrator.")
        return active_root

    with st.expander("Upload & process a new dataset", expanded=False):
        st.caption("Upload a raw CSV → validate the 25-column source contract → run the full Common + Branch A + Branch B workflow in an isolated workspace.")
        upload=st.file_uploader("Raw AI job-market CSV",type=["csv"],key="global_dataset_upload")
        if upload is None:
        ...
        with st.expander("Expected vs detected columns",expanded=not report["valid"]):
            st.dataframe(report["field_table"],use_container_width=True,hide_index=True,height=430)
        ...
        if process:
            ...
            with st.spinner("Running full data-quality, segmentation and salary-prediction workflow…"):
                try:
            ...
            st.session_state.active_run_label=f"{run_id} · {summary['raw_shape'][0]} rows · {summary['best_model']}"
