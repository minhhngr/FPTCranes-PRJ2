...
def _filters(st, df):
    with st.expander("Interactive filters", expanded=True):
        ...
        cats = c1.multiselect(
            "Job category", sorted(df.job_category.dropna().astype(str).unique()), key="p1_cat"
        )
        ...
        countries = c3.multiselect(
            "Country", sorted(df.country.dropna().astype(str).unique()), key="p1_country"
        )
        remote = c4.multiselect(
            "Remote work", sorted(df.remote_work.dropna().astype(str).unique()), key="p1_remote"
        )
...
def render(st, root, role="admin"):
    ...
    st.caption(
        "Common Foundation — Stage 1 Project Scope & Raw Data Ingestion → Stage 2 Basic Clean → Stage 3 Contradictory-Feature Investigation"
    )
    ...
    metric_cols(
        st,
        [
            ("Raw records", f"{audit['raw_rows']:,}"),
            ("Raw columns", audit["raw_columns"]),
            ("Clean records", f"{audit['clean_rows']:,}"),
            ("Clean columns", audit["clean_columns"]),
            ("Removed corrupted rows", audit["invalid_category_rows_removed"]),
        ],
    )
    ...
    with tabs[0]:
        ...
        st.markdown(
            '<div class="stage-note">Target <b>annual_salary_usd</b>. The raw file is parsed and profiled before model decisions. Salary extremes are retained unless there is evidence that they are invalid.</div>',
        ...
        with c1:
            fig = px.histogram(
                ...
                nbins=24,
                marginal="box",
                title="Observed Annual Salary Distribution",
                labels={"annual_salary_usd": "Annual salary (USD)"},
            )
            fig.add_vline(
                x=float(filtered.annual_salary_usd.mean()),
                line_dash="dash",
                annotation_text=f"Mean {money(filtered.annual_salary_usd.mean())}",
            )
            fig.add_vline(
                x=float(filtered.annual_salary_usd.median()),
                line_dash="dot",
                annotation_text=f"Median {money(filtered.annual_salary_usd.median())}",
            )
        ...
        with c2:
            ...
            st.dataframe(
                pd.DataFrame(
                    {
                        "Metric": ["Rows", "Min", "Q1", "Median", "Mean", "Q3", "Max", "Std"],
                        "Value": [
                            f"{len(filtered):,}",
        ...
        skew_note = (
            ...
            else "left-skewed or symmetric"
        )
        interpretation_card(
            st,
            f"Filtered salary mean is {money(filtered.annual_salary_usd.mean())} versus median {money(filtered.annual_salary_usd.median())}; range is {money(filtered.annual_salary_usd.min())}–{money(filtered.annual_salary_usd.max())}.",
            f"The target is {skew_note}. Extreme salaries are visible but are not removed automatically because the chart alone does not prove data corruption.",
            "Retain target extremes for now; handle any outlier policy on TRAIN only and validate impact on temporal performance.",
            "info",
        )
    ...
    with tabs[1]:
        ...
        with c1:
            ...
            fig = px.bar(
                ...
                orientation="h",
                text="unique",
                title="Top Feature Cardinalities — Raw Data",
            )
        ...
        with c2:
            ...
            fig = px.bar(
                ...
                orientation="h",
                text="total_missing",
                title="Missing / Hidden-Missing Audit by Feature",
            )
        ...
        interpretation_card(
            st,
            f"Basic Clean changed {audit['raw_rows'] - audit['clean_rows']} row(s) and {audit['raw_columns'] - audit['clean_columns']} column(s); raw missing/hidden-missing count is {total_missing}.",
            "Structural cleanliness alone is not enough: the dataset can still contain semantic contradictions even when missingness and duplication are low.",
            "Proceed to Stage 3 contradiction checks before trusting any feature for modeling.",
    ...
    with tabs[2]:
        ...
        fig = px.bar(
            ...
            orientation="h",
            text="affected_pct",
            title="Data Logic & Integrity Issues — Ranked by Severity",
            labels={"affected_pct": "Affected records (%)", "issue": "Issue"},
        )
        ...
        interpretation_card(
            st,
            f"Largest logic finding: {top_issue.issue} affects {top_issue.affected_pct:.1f}% ({int(top_issue.affected_rows):,} rows).",
            "This is a semantic-integrity problem, not a missing-value problem; the affected field family must be governed before preprocessing.",
            str(top_issue.required_action),
            "warning",
        )

        s1, s2, s3 = st.tabs(
            ["Experience contradiction", "Salary range inconsistency", "Salary tier inconsistency"]
        )
        with s1:
            c1, c2 = st.columns(2)
            with c1:
                fig = px.box(
                    ...
                    points="outliers",
                    title="Years of Experience by Experience Level",
                )
            ...
            fig.update_layout(
                title="Annual Salary by Raw Years of Experience",
            ...
            interpretation_card(
                st,
                f"Experience-level mean-salary spread is {money(spread)} while raw years-of-experience correlation with salary is {corr:+.2f}.",
                "If bucket labels and numeric years tell materially different stories, they should not both be trusted as interchangeable experience signals.",
                "Keep the contradiction visible and let ablation decide which signal adds stable out-of-time value.",
                "warning",
            )
        with s2:
            if not flags.empty:
                ...
                with c1:
                    ...
                    fig = px.bar(
                        ...
                        y="records",
                        text="records",
                        title="Salary Range Audit — Status",
                    )
                    fig.update_traces(textposition="outside")
                ...
                with c2:
                    fig = px.histogram(
                        ...
                        nbins=25,
                        barmode="overlay",
                        title="Actual Salary Distribution by Range Status",
                    )
                ...
                fig.add_trace(
                    go.Scatter(
                        x=sample.row, y=sample.salary_min_usd, mode="lines", name="Stated min"
                    )
                )
                fig.add_trace(
                    go.Scatter(
                        x=sample.row, y=sample.salary_max_usd, mode="lines", name="Stated max"
                    )
                )
                fig.add_trace(
                    go.Scatter(
                ...
                fig.update_layout(
                    title="Actual Salary vs Stated Salary Bounds",
                    xaxis_title="Records sorted by actual salary",
                    yaxis_title="USD",
                )
                ...
                interpretation_card(
                    st,
                    f"{rate:.1f}% of filtered records fall outside the stated salary_min_usd / salary_max_usd range.",
                    "The range fields are target-adjacent metadata and cannot be treated as clean predictors when they disagree with the target this often.",
                    "Block salary_min_usd and salary_max_usd from the primary prediction feature set; retain them only for audit.",
                    "warning",
                )
        with s3:
            if not flags.empty:
                ...
                d["tier_status"] = np.where(
                    d.salary_tier_mismatch.astype(bool), "Mismatch", "Match"
                )
                c1, c2 = st.columns(2)
                with c1:
                    g = (
                    ...
                    fig = px.bar(
                        ...
                        y="records",
                        text="records",
                        title="Salary Tier Consistency — Status",
                    )
                ...
                with c2:
                    ...
                    fig = px.imshow(
                        ...
                        text_auto=True,
                        aspect="auto",
                        title="Stated Tier vs Expected Tier from Annual Salary",
                        labels=dict(x="Expected tier", y="Stated tier", color="Records"),
                    )
                ...
                fig = px.box(
                    ...
                    color="tier_status",
                    points=False,
                    title="Observed Salary Distribution by Stated Tier",
                )
                show_plot(st, fig, "p1_tier_box")
                rate = 100 * d.salary_tier_mismatch.astype(bool).mean()
                interpretation_card(
                    st,
                    f"{rate:.1f}% of filtered records have a stated salary tier that disagrees with the tier implied by annual_salary_usd.",
                    "salary_tier behaves like a noisy derivative of the target, creating both leakage risk and semantic inconsistency.",
                    "Exclude salary_tier from model X and use it only as a diagnostic field.",
                    "warning",
                )
    ...
    with tabs[3]:
        ...
        chart = chart_selector(
            ...
            "Choose chart",
            [
                "Salary by job domain",
                "Salary by country",
                "Salary by city",
                "Salary by years of experience",
                "Demand score vs salary",
        ...
        if chart == "Salary by job domain":
            ...
            fig = px.bar(
                ...
                text="mean_salary",
                hover_data=["records"],
                title="Mean Salary by Job Domain",
            )
            fig.update_traces(texttemplate="$%{text:,.0f}", textposition="outside")
            ...
            fig = px.line(
                ...
                labels={"value": "Salary (USD)", "variable": "Metric"},
            )
            ...
            fig = px.scatter(
                ...
                title="Demand Score vs Annual Salary",
                trendline=None,
            )
        else:
            fig = px.scatter(
                ...
                color="job_category",
                hover_data=["job_title", "country", "years_of_experience"],
                title="Benefits Score vs Annual Salary",
            )
        show_plot(st, fig, "p1_explorer")
        st.caption(
            f"Current chart uses {len(filtered):,} filtered records. Hover over marks to inspect exact values."
        )
        if chart in {"Salary by job domain", "Salary by country", "Salary by city"}:
            ...
            label_col = (
                ...
                else ("country" if chart == "Salary by country" else "city")
            )
            interpretation_card(
                st,
                f"Highest mean salary in the current chart is {top[label_col]} at {money(top.mean_salary)} across {int(top.records):,} record(s).",
                "This is a descriptive filtered comparison; unequal sample sizes and synthetic data structure can drive apparent gaps.",
                "Use hover counts and filters to check support before interpreting category differences.",
                "info",
            )
        elif chart == "Salary by years of experience":
            corr = (
            ...
            interpretation_card(
                st,
                f"Current-filter Pearson correlation between years_of_experience and salary is {corr:+.2f}.",
                "The direction is diagnostic only and may reflect the known synthetic-looking experience artifact.",
                "Compare this chart with Stage 5 ablation and temporal CV rather than treating it as a labor-market law.",
                "warning",
            )
        else:
            ...
            interpretation_card(
                st,
                f"Current-filter correlation between {xcol} and salary is {corr:+.2f}.",
                "Scatter structure shows association strength and heterogeneity, not causality.",
                "Use this as exploratory evidence only; model selection remains based on temporal validation.",
                "info",
            )
