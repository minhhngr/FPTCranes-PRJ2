...
def render(st, root, role="admin"):
    ...
    st.caption(
        "Branch B — B7 Deployment & Prediction Output | Fast Inference using Top 2 Features: Job Category & Years of Experience"
    )
    ...
    if res3_file.exists():
        with st.expander(
            "🎯 Quick Load from 3 Reserved Test Records (Pre-configured Scenarios)", expanded=True
        ):
            ...
            st.dataframe(
                r3[disp_cols].style.format({"annual_salary_usd": "${:,.0f}"}),
                use_container_width=True,
                hide_index=True,
            )
            c_l1, c_l2, c_l3 = st.columns(3)
            for idx, (col_btn, row_data) in enumerate(
                zip([c_l1, c_l2, c_l3], r3.to_dict(orient="records"))
            ):
                with col_btn:
                    if st.button(
                        f"Load Row {idx + 1}: {row_data.get('job_title', '')}",
                        key=f"p6_btn_load_{idx}",
                        use_container_width=True,
                    ):
                        rec = {
                        ...
                        st.success(
                            f"Loaded Row {idx + 1} ({rec['job_category']}, {rec['years_of_experience']} yrs) into Queue! Actual: {money(rec['actual_salary_usd'])}"
                        )
                        st.rerun()
    ...
    st.caption(
        "Salary prediction runs exclusively on the 2 core drivers: **Job Category** and **Years of Experience**."
    )
    with st.form("prediction_form", clear_on_submit=False):
        ...
        with c2:
            years = st.slider(
                "Years of experience (Primary Feature 2)",
                int(nr["years_of_experience"]["min"]),
                int(nr["years_of_experience"]["max"]),
                int(round(nr["years_of_experience"]["median"])),
            )
    ...
    if add:
        ...
        st.session_state.prediction_queue.append(rec)
        st.success(
            f"Added scenario: {job_category} ({years} years experience). Queue now contains {len(st.session_state.prediction_queue)} scenario(s)."
        )
    ...
    if st.session_state.prediction_queue:
        ...
        show_cols = [
            c
            for c in ["job_title", "job_category", "years_of_experience", "actual_salary_usd"]
            if c in q.columns
        ]
        if not show_cols:
        ...
        st.download_button(
            "Download Queue in Original Schema",
            source_export.to_csv(index=False).encode("utf-8"),
            file_name="ai_job_scenarios_2features.csv",
            mime="text/csv",
            use_container_width=True,
        )
    else:
    ...
    if "last_prediction_batch" in st.session_state:
        ...
        st.dataframe(
            r.style.format(format_dict, na_rep="-"), use_container_width=True, hide_index=True
        )

        st.markdown("### Batch Prediction Visualization")
        view = st.selectbox(
            "Prediction chart",
            ["Point estimate + error band", "Salary by job category", "Salary vs experience"],
            key="p6_chart",
        )
        rr = r.reset_index(drop=True).copy()
        rr["scenario"] = [
            f"{row.get('job_category', 'Scenario')} ({row.get('years_of_experience', 0)}y)"
        ...
        if view == "Point estimate + error band":
            ...
            fig.update_layout(
                title="Predicted Salary with Empirical Validation-error Band (±$34,101)",
                yaxis_title="USD",
            )
            ...
            fig = px.bar(
                ...
                title="Mean Predicted Salary by Job Category",
            )
            ...
            fig = px.scatter(
                ...
                y="predicted_salary_usd",
                color="job_category",
                hover_data=["job_category", "years_of_experience"],
        ...
        spread = (
            float(r.predicted_salary_usd.max() - r.predicted_salary_usd.min())
            if len(r) > 1
            else 0.0
        )
        ...
        st.download_button(
            ...
            file_name="salary_prediction_results.csv",
            mime="text/csv",
        )
        st.warning(
            f"Displayed ±{money(q90_error_band)} is an empirical 90th-percentile absolute validation-error band, not a formal confidence interval or salary guarantee."
        )
    ...
    if top2_ex_file.exists():
        with st.expander("Representative locked-test examples from Top 2 Model run"):
            ...
            st.dataframe(
                ex[cols].style.format(
                    {
                        c: "${:,.0f}"
                        for c in ["annual_salary_usd", "predicted_salary_usd", "absolute_error_usd"]
                        if c in cols
                    }
                ),
                use_container_width=True,
