...
def downloadable_table(st, df: pd.DataFrame, title: str, key: str, file_name: str|None=None, height=350):
    st.markdown(f"#### {title}")
    st.dataframe(df, use_container_width=True, hide_index=True, height=height)
    st.download_button("Download CSV", df.to_csv(index=False).encode("utf-8"), file_name=file_name or f"{key}.csv", mime="text/csv", key=f"dl_{key}")


def ranked_bar(df, category, value, title, value_prefix="", value_suffix="", horizontal=True, top_n=None):
    d=df.copy()
    ...
    if value_prefix or value_suffix:
        fig.update_traces(texttemplate=f"{value_prefix}%{{text:,.0f}}{value_suffix}", textposition="outside")
    else:
...
def dynamic_family_comment(assign: pd.DataFrame, family: str) -> str:
    if assign.empty: return "No data available for the current filters."
    if family=="Job Domain":
        g=assign.groupby("cluster")["job_category"].agg(lambda s:s.value_counts().index[0]).to_dict()
        return "Dominant job-domain labels by cluster: " + "; ".join(f"C{k}: {v}" for k,v in sorted(g.items())) + "."
    if family=="Skills":
        s=assign.groupby("cluster")["skill_count"].mean().round(1).to_dict()
        return "Average normalized skill count by cluster: " + "; ".join(f"C{k}: {v}" for k,v in sorted(s.items())) + "."
    if family=="Experience":
        s=assign.groupby("cluster")["years_of_experience"].mean().round(1).to_dict()
        return "Mean years of experience by cluster: " + "; ".join(f"C{k}: {v}" for k,v in sorted(s.items())) + "."
    if family=="Company":
        g=assign.groupby("cluster")["company_size"].agg(lambda s:s.value_counts().index[0]).to_dict()
        return "Most common company size by cluster: " + "; ".join(f"C{k}: {v}" for k,v in sorted(g.items())) + "."
    if family=="Geography":
        g=assign.groupby("cluster")["country"].agg(lambda s:s.value_counts().index[0]).to_dict()
        return "Most common country by cluster: " + "; ".join(f"C{k}: {v}" for k,v in sorted(g.items())) + "."
    if family=="Demand / Benefits":
        g=assign.groupby("cluster")[["demand_score","benefits_score_10"]].mean().round(1)
        return "Demand/benefits means by cluster: " + "; ".join(f"C{i}: demand {r.demand_score}, benefits {r.benefits_score_10}" for i,r in g.iterrows()) + "."
    return ""


def interpretation_card(st, observation: str, interpretation: str, action: str | None = None, tone: str = "info", title: str = "Data-driven interpretation"):
    ...
    bg = {"info":"#f4f9ff","success":"#f2fbf5","warning":"#fff9ed","error":"#fff2f2"}.get(tone,"#f4f9ff")
    border = {"info":"#9bc8f2","success":"#9bd7ad","warning":"#f3c96b","error":"#ef9b9b"}.get(tone,"#9bc8f2")
