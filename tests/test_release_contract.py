...
from ai_job_market.core import SOURCE_COLUMNS, MODEL_FEATURES, TARGET, normalize_skills
...
def test_temporal_holdout_isolated():
    ...
    s=json.load(open(ROOT/"outputs/02_data_ready_for_ml/temporal_split_summary.json",encoding="utf-8"))
...
def test_segmentation_target_excluded_and_dimension_traceable():
    meta=json.load(open(ROOT/"outputs/03_ai_job_market_segmentation/segmentation_metadata.json",encoding="utf-8"))
...
def test_model_comparison_includes_required_baselines_and_runtime():
    ...
    expected={"Dummy Median","Linear Regression","Ridge Regression","Random Forest","Gradient Boosting"}
...
def test_segmentation_k_selection_is_explicit_stable_and_reproducible():
    ...
    required={"stability_ari","min_cluster_share","eligible","within_primary_tolerance","selected"}
...
def test_legacy_main_archive_manifest_packaged():
    ...
    for token in ["training/temporal_cv.py","training/ablation_study.py","model_training_comparison.py","best_model_selection_feature_importance_review.py"]:
